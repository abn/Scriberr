...
// WhisperAPIAdapter implements the TranscriptionAdapter interface for an external Whisper API
type WhisperAPIAdapter struct {
	*BaseAdapter
}

// NewWhisperAPIAdapter creates a new Whisper API adapter
func NewWhisperAPIAdapter() *WhisperAPIAdapter {
...
// GetSupportedModels returns a list of supported model strings
func (a *WhisperAPIAdapter) GetSupportedModels() []string {
	return []string{"whisper-1"}
}

// PrepareEnvironment ensures the model environment is ready
func (a *WhisperAPIAdapter) PrepareEnvironment(ctx context.Context) error {
	a.initialized = true
	return nil
}

// IsReady checks if the API is reachable
func (a *WhisperAPIAdapter) IsReady(ctx context.Context) bool {
	if !a.initialized {
		return false
	}

	// This function requires a URL, but we don't have access to params here.
	// We'll assume it's ready if initialized. The actual request will fail if it's down.
	return true
}

// GetEstimatedProcessingTime provides estimated processing time
...
func (a *WhisperAPIAdapter) Transcribe(ctx context.Context, input interfaces.AudioInput, params map[string]interface{}, procCtx interfaces.ProcessingContext) (*interfaces.TranscriptResult, error) {
	...
	apiUrl := a.GetStringParameter(params, "api_url")
	if apiUrl == "" {
		writeLog("Error: api_url parameter is required but not provided")
		return nil, fmt.Errorf("api_url parameter is required")
	}

	apiKey := a.GetStringParameter(params, "api_key")
...
// init registers the Whisper API adapter
func init() {
	registry.RegisterTranscriptionAdapter("whisper_api", NewWhisperAPIAdapter())
}
